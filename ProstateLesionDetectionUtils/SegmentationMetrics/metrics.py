...
#code for the average surface distance, dice score, hausdorff and surfaces from  https://www.deepmind.com/
...
from ProstateLesionDetectionUtils.SegmentationMetrics import lookup_tables
...
def metrics_all_folds(path,fold):
    if fold:
        for index,file in enumerate(glob(path+"\confusion_matrix*.csv")):
            matr=pd.read_csv(file)
            matr["Sensitivity"]=(matr["# True positive"]/(matr["# True positive"]+matr["False negative"]))
            matr["Specificity"]=(matr["True negative"]/(matr["True negative"]+matr["False positive"]))
            matr["Balanced accuracy"]=(matr["Sensitivity"]+matr["Specificity"])/2
            matr["Precision"]=(matr["# True positive"]/(matr["# True positive"]+matr["False positive"]))
            matr["False positive rate"]=1-matr["Specificity"]
            matr["False negative rate"]=1-matr["Sensitivity"]
            matr["Prevelance Threshold"]=(matr["False positive rate"]**0.5)/(((matr["Sensitivity"])**0.5)+(matr["False positive rate"]**0.5))
            matr["Fold"]="Fold{}".format(index+1)
            matr.to_csv(path+"\metrics{}.csv".format(index+1))
        for index,file in enumerate(glob(path+"\all_metr*.csv")):
            matr=pd.read_csv(file,delimiter=",",header=1)
            matr["Fold"]="Fold{}".format(index+1)
            matr.to_csv(path+"\\all_metr{}.csv".format(index))
        df1=pd.read_csv(path+"\metrics1.csv")
        df2=pd.read_csv(path+"\metrics2.csv")
        df3=pd.read_csv(path+"\metrics3.csv")
        df4=pd.read_csv(path+"\metrics4.csv")
        df5=pd.read_csv(path+"\metrics5.csv")
        frames1 = [df1, df2, df3, df4, df5]
        result1 = pd.concat(frames1)
        me1=pd.read_csv(path+"\\all_metr1.csv")
        me2=pd.read_csv(path+"\\all_metr2.csv")
        me3=pd.read_csv(path+"\\all_metr3.csv")
        me4=pd.read_csv(path+"\\all_metr4.csv")
        me5=pd.read_csv(path+"\\all_metr5.csv")
        frames2 = [me1,me2,me3,me4,me5]
        result2 = pd.concat(frames2)
        #result1.join(result2,lsuffix="FOLD_DICE",rsuffix="FOLD_ACC")
        total=pd.concat([result1,result2],axis=1)
        total.rename(columns={"# True positive":"True positive"},inplace=True)
        total=total.drop(columns="Unnamed: 0")
        total.to_csv(path+"\\all_metrics_all_folds.csv")
    else:
        for index,file in enumerate(glob(path+"\confusion_matrix*.csv")):
            matr=pd.read_csv(file)
            matr["Sensitivity"]=(matr["# True positive"]/(matr["# True positive"]+matr["False negative"]))
        ...
        for index,file in enumerate(glob(path+"\all_metr*.csv")):
            ...
            matr["Fold"]="Fold{}".format(index+1)
            matr.to_csv(path+"\\all_metr.csv".format(index))
        df1=pd.read_csv(path+"\metrics.csv")

        frames1 = df1

        me1=pd.read_csv(path+"\\all_metr.csv")

        frames2 = me1

        #result1.join(result2,lsuffix="FOLD_DICE",rsuffix="FOLD_ACC")
        total=pd.concat([frames1,frames2],axis=1)
        total.rename(columns={"# True positive":"True positive"},inplace=True)
        total=total.drop(columns="Unnamed: 0")
        total.to_csv(path+"\\all_metrics_all_folds.csv")
...
def confusion_matrix(gt,pred):
    ...
    gt=gt.astype("int")
    tp,fp,tn,fn=0,0,0,0
    confusion_matr=np.zeros([4])
    for k in range(pred.shape[0]):
        for l in range(pred.shape[1]): 
            if (pred[k][l]==1 and gt[k][l]==1):
                tp=tp+1
            elif (pred[k][l]==1 and gt[k][l]==0):
                fp=fp+1
            elif (pred[k][l]==0 and gt[k][l]==1):
                fn=fn+1
            elif (pred[k][l]==0 and gt[k][l]==0):
                tn=tn+1
    confusion_matr[0],confusion_matr[1],confusion_matr[2],confusion_matr[3]=tp,fp,fn,tn
