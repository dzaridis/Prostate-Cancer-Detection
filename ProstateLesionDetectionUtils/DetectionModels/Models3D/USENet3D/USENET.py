...
import time
from ProstateLesionDetectionUtils.DetectionModels.Models3D.USENet3D.EncodersDecoders import *
tf.compat.v1.logging.set_verbosity(tf.compat.v1.logging.ERROR)
#tf.config.run_functions_eagerly(True)

class USENetMod(tf.keras.Model):
    def __init__(self, num_filters, pool_size, **kwargs):
        self.kernel_size = kwargs.get('kernel_size', [3 for _ in range(len(num_filters))])
        super(USENetMod, self).__init__()
        self.enc_parts = []
        self.dec_parts = []
...
class TrainUSEnet:
    ...
    def ModelBuild(self):
        ...
        self.model = USENetMod(self.params["FILTERS"], self.params["POOL_SIZE"], kernel_size = self.params["kernel_size"])
